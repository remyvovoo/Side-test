...
- **Identification des infos par IA** (chantier C), en deux passes dans `/api/analyze-card` :
  1. **Lecture de la photo** (vision, sortie structurée) : nom, numéro, série si imprimée, langue, rareté si le symbole est net.
  2. **Recherche en ligne à partir du code** (`web_search`) : le numéro imprimé (ex. « 052/196 ») identifie la carte de façon quasi unique dans les bases de vendeurs (Cardmarket et consorts) — utilisé pour retrouver la **série** et la **rareté** quand la photo seule ne les donne pas (le nom de l'extension n'est presque jamais imprimé sur la carte). Best-effort : si la recherche échoue ou ne conclut pas, l'extraction de la photo reste acquise.
  Lancée automatiquement à la validation du recto (pendant que le vendeur fait le verso) ; ne remplit que les champs **encore vides** du studio — jamais la saisie du vendeur. Bandeau d'état + bouton « Relancer » dans « Infos de ma carte », qui précise si un champ vient de la photo ou d'une recherche en ligne (à vérifier en priorité dans ce cas). Consigne stricte au modèle dans les deux passes : **ne jamais compléter de mémoire**, chaîne vide si le doute persiste — une annonce inventée est pire qu'un champ à remplir à la main.
  Plafond d'essai par compte : `User.aiAnalysisCount` vs `AI_ANALYSIS_LIMIT` (30 par défaut), décompté une seule fois par carte (pas par passe), uniquement si l'extraction photo réussit.
  Modèle réglable sans redéploiement via `ANTHROPIC_MODEL` (défaut `claude-opus-5`) — utile en cas de saturation passagère du modèle par défaut côté Anthropic, déjà rencontrée en test (tout Opus 5 renvoyait 529 pendant ~20 min ; Sonnet 5 et Haiku répondaient normalement). Repères mesurés sur une vraie carte (Pikachu 052/196) : lecture photo seule ~2 centimes/6-23 s avec Opus 5 en effort `low` (Haiku : ~0,4 centime/4,5 s mais rate la rareté) ; passe recherche en ligne testée avec Sonnet 5 ~9 centimes/22-36 s (beaucoup de tokens de résultats de recherche). Total réaliste quand les deux passes tournent : autour de **10-12 centimes et jusqu'à ~40 s par carte**. Le budget serveur (`maxDuration = 60`) tient, mais c'est sensiblement plus cher/lent qu'annoncé initialement — arbitrage à revoir avec Remy si le volume grossit (ex. limiter la recherche en ligne à un ou deux essais par carte, ou la rendre optionnelle).
