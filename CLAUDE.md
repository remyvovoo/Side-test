...
- **Identification des infos par IA** (chantier C), en deux passes dans `/api/analyze-card` :
  ...
  Modèle réglable sans redéploiement via `ANTHROPIC_MODEL` (défaut `claude-opus-5`) — utile en cas de saturation passagère côté Anthropic (déjà rencontrée en test à deux reprises : tout Opus 5 renvoyait 529 pendant 20-30 min pendant que Sonnet 5 et Haiku répondaient normalement — bascule temporaire utilisée pour continuer les tests, jamais laissée en prod). Repère mesuré sur une vraie carte (Pikachu 052/196), les deux passes réunies, hors incident Opus : **~5 s au total** (Claude vision seul, TCGdex ajoute quelques centaines de ms) — bien loin des ~10-40 s d'une première version qui passait par une recherche web confiée au modèle, abandonnée pour cette raison.
