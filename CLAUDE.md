...
- **Recadrage manuel = découpe réelle au quadrilatère** (`CropScreen.applyCrop`) : déplacer une poignée retaille vraiment la carte (clip polygonal). Avant, seul un recadrage rectangulaire était fait — invisible sur une carte détourée entourée de transparence, Remy l'avait repéré. La rectification de perspective (déformer le quadrilatère vers un rectangle droit) reste NON faite — même statut « mis de côté par choix » que plus bas.
